/** ----------------------------------------------------------------------------
 * @file: vtec_u4_simulation_node.cpp
 * @date: April 10, 2022
 ...
 * 
 * @brief: ROS simulation node for the VTec U-IV.
...
{
    ros::init(argc, argv, "vtec_u4_simulation_node");
    ...
    ros::Publisher  uuv_eta_pose  = private_nh.advertise<vanttec_msgs::EtaPose>("/uuv_simulation/dynamic_model/eta_pose_", 10);
    ...
    while(ros::ok())
    {
        ...
        uuv_eta_pose.publish(uuv_model.eta_pose_);
