...
class uuv_nav:
    def __init__(self):
        ...
        #Camera parameters
        rospy.Subscriber("/frontr200/camera/color/camera_info", CameraInfo, self.cameraCB, queue_size=1)
        ...
        self.gate_police_u_pixel = 0
        self.gate_police_v_pixel = 0
        self.gate_police_depth = 0

        self.gate_gangster_u_pixel = 0
        self.gate_gangster_v_pixel = 0
        self.gate_gangster_depth = 0
    ...
    def detected_objects_callback(self, msg):
        ...
        for object in msg.objects:
            if   object.clase == "gun":
                ...
                #Pixel central del objeto
                self.gate_police_u_pixel = object.X
                self.gate_police_v_pixel = object.Y
                
                #Distancia del objeto al UUV
                self.gate_police_depth = object.Depth

                #Marcar objeto como encontrado
                self.gate_class_found = 1
            elif object.clase == "gangster" and self.completedMissions["GATE"] == 0:
                #Pixel central del objeto
                self.gate_gangster_u_pixel = object.X
                self.gate_gangster_v_pixel = object.Y

                #Distancia del objeto al UUV
                self.gate_gangster_depth = object.Depth
    ...
    def retos(self):
        ...
        if self.gate_class_found == 1 and self.gate_found == 1:

            #Pasar por el lado del policia
            if self.uuv.side == "police":
                self.gate(self.gate_police_u_pixel, self.gate_police_v_pixel, self.gate_police_depth)

            #Pasar por el lado del gangster
            else:
                self.gate(self.gate_gangster_u_pixel, self.gate_gangster_v_pixel, self.gate_gangster_depth)
    ...
    def cameraCB(self, msg):
        #Definir los datos de la camara
        if self.cameraInfo != True:
            msg.D = [-0.423807, 0.228076, -0.004043, -0.001038, 0]
            msg.K = [463.889, 0, 320, 0, 463.889, 240, 0, 0, 1]
            msg.R = [0.999864, 0.008003, -0.014412, -0.007971, 0.999966, 0.00223, 0.01443, -0.002115, 0.999894]
            #msg.P = [593.55, 0, 352.672, 0, 0, 593.55, 273.193, 0, 0, 0, 1, 0]
            msg.P = [463.889, 0, 320, 0, 0, 463.889, 240, 0, 0, 0, 1, 0]
            self.camera.fromCameraInfo(msg)
            self.cameraInfo = True
            rospy.loginfo(msg)
    ...
    def pixelto3D(self, u, v, d):
        ...
        (z, y, x) = [el * d/1000 for el in ray]
        
        return (x, y, z)

    def gate(self, u, v, d):
        #Si los datos de la camara ya estan definidos
        if self.cameraInfo:
            ...
            #Obtener coordenada en x, y, z del gate
            (gate_x, gate_y, gate_z) = self.pixelto3D(self.gate_u_pixel, self.gate_v_pixel, d)

            #Convertir pasar a world frame
            gate_x = gate_x + self.uuv.ned_x
            gate_y = gate_y + self.uuv.ned_y
            gate_z = self.uuv.ned_z

            #TODO
            #Obtener coordenadas de los extremos del gate y sacar un offset en base a la coordenada central del gate, restar ese offset a la promedio 

            #Left coordinate of the gate
            (gateL_x, gateL_y, gateL_z) = self.pixelto3D(self.gate_x_pixel, v, d)

            #Right coordinate of the gate
            (gateR_x, gateR_y, gateR_z) = self.pixelto3D((self.gate_x_pixel + self.gate_w_pixel), v, d)

            #Obtener coordenada promedio
            midX = (gateL_x + gateR_x) / 2
            midY = (gateL_y + gateR_y) / 2
            midZ = (gateL_z + gateR_z) / 2


            #Imprimir coordenada del objetivo y del punto medio del gate
            rospy.loginfo("Objetivo X = " + str(x) + " Y = " + str(y) + " Z = " + str(z))
            rospy.loginfo("Gate mid X = " + str(gate_x) + " Y = " + str(gate_y) + " Z = " + str(gate_z))
            rospy.loginfo("Gate mid avg X = " + str(midX) + " Y = " + str(midY) + " Z = " + str(midZ))
            ...
            rospy.loginfo("Diferencia X = " + str(diffX) + " Y = " + str(diffY) + " Z = " + str(diffZ)) 


            #Imprimir coordenada promedio
            rospy.loginfo("Promedio X = " + str(avgX) + " Y = " + str(avgY) + " Z = " + str(avgZ)) 

            #Definir coordenada a donde ira el uuv
            self.point_handler.x = avgX
            self.point_handler.y = avgY
            self.point_handler.z = round(self.uuv.ned_z,2)
