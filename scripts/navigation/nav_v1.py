...
class uuv_instance:
    ...
    def cluster_callback(self,msg):
        ...
        cluster=[tuple(y) for y in msg.clusters]
        setCluster=set(cluster)
        if  setCluster!=self.oldclusters:
            self.newclusters=setCluster.symmetric_difference(self.oldclusters)
            self.oldclusters=setCluster
    ...
    def search(self, uuv):
        ...
        if (self.search_control == 0):
            ...
            if self.search_step == 0:
                ...
                rospy.sleep(3)
                if uuv.newclusters!=uuv.oldclusters:
